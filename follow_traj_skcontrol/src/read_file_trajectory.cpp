...
int main(int argc, char **argv){
    ros::init(argc, argv, "follow_traj_skcontrol");
    ros::NodeHandle nh;
    ...
    std::string x_d, y_d, theta_d, speed_d;
    
    ss >> x_d;
	while(!ss.eof()){	   	  	
        ss >> y_d >> theta_d >> speed_d;
	
        trajectory_points.push_back(stod(x_d));
        trajectory_points.push_back(stod(y_d));
        trajectory_points.push_back(stod(theta_d));
        trajectory_points.push_back(stod(speed_d));
        
        if(DEBUG) ROS_INFO("desire_x: %.3f, desire_y:%.3f, desire_theta: %.3f, desire_speed: %.3f",stod(x_d),stod(y_d),stod(theta_d),stod(speed_d));	
	    ss >> x_d;
